...
- `framework/src/context/mod.rs` — `Context` facade + `ContextStore` + `CONTEXT` task_local (Laravel-shaped per-request key/value bag)
...
## Task 21: Context facade — key/value scoped to a request

`Context::add(key, value)` for arbitrary request-scoped state that propagates
through every spawned future, including queued jobs that hydrate the context
on resume. The `RequestId` middleware seeds the bag with `_request_id` so log
correlation continues to work automatically. The facade matches Laravel's
`Context` shape (`add`/`get`/`push`/`has`/`forget`/`all`/`hidden_add`/`hidden_get`)
so a Laravel dev recognizes the API on sight.

**Files:**
- Create: `framework/src/context/mod.rs`
- Modify: `framework/src/lib.rs` — declare module + re-export `Context`
- Modify: `framework/src/logging/request_id.rs` — `RequestIdMiddleware` seeds Context with `_request_id`
...
mod tests {
    ...
    async fn hidden_storage_is_separate_from_visible() {
        CONTEXT
            .scope(ContextStore::default(), async {
                ...
                assert_eq!(Context::hidden_get::<String>("secret_key"), Some("shh".into()));
    ...
    async fn outside_scope_operations_are_silent_noops() {
        // Calling Context::add outside a scope must not panic — middleware
        // didn't run yet (e.g. early-boot code, tests without setup).
        Context::add("k", "v");
        assert_eq!(Context::get::<String>("k"), None);
    }

    #[tokio::test]
    async fn middleware_seeds_request_id_into_context() {
        use crate::logging::request_id::{RequestId, REQUEST_ID};
        let id = RequestId::new();
        let id_str = id.as_str().to_string();
        ...
        CONTEXT
            .scope(ContextStore::default(), async move {
                REQUEST_ID
                    .scope(id, async move {
                        // Simulate what RequestIdMiddleware does after Task 4 edit.
                        if let Some(id) = crate::logging::request_id::current_request_id() {
                            Context::add("_request_id", id.as_str().to_string());
                        }
                        *captured_clone.lock().unwrap() =
                            Context::get::<String>("_request_id");
                    })
                    .await;
            })
            .await;

        assert_eq!(captured.lock().unwrap().clone(), Some(id_str));
    }
}
```
...
```bash
cargo test -p suprnova context::
```

Expected: FAIL with "cannot find Context".
...
impl Context {
    ...
    /// Set `key` to `value` in the hidden bag.
...
```rust
// framework/src/lib.rs — additions
pub mod context;
pub use context::{Context, ContextStore};
```

- [ ] **Step 5: Seed `_request_id` from middleware**

```rust
// framework/src/logging/request_id.rs — extend RequestIdMiddleware::handle
// AFTER computing `id` but BEFORE the REQUEST_ID::scope call, wrap both
// scopes so Context::add picks up the id automatically.
use crate::context::{Context, ContextStore, CONTEXT};

// ... inside handle(...):
let store = ContextStore::default();
let id_for_seed = id_str.clone();
let result = CONTEXT
    .scope(store, async move {
        Context::add("_request_id", id_for_seed);
        REQUEST_ID
            .scope(id.clone(), async move { next(request).await })
            .await
    })
...
```bash
cargo test -p suprnova context::
cargo test -p suprnova logging::
```

Expected: all 6 Context tests pass; existing RequestId tests still pass.

- [ ] **Step 7: Commit**

```bash
git add framework/src/context/mod.rs framework/src/lib.rs framework/src/logging/request_id.rs
git commit -m "feat(context): Laravel-shaped Context facade scoped per request"
```

---

## Task 22: Polish — workspace lint + final verification
...
- `Context` facade — Laravel-shaped per-request key/value bag with visible + hidden storage
...
| Laravel-shaped `Context` facade (`add`/`get`/`push`/`has`/`forget`/`all`/`hidden_*`) | Task 21 |
| `_request_id` auto-seeded into Context by middleware | Task 21 |
...
**Type consistency:** `EventTrait` consistent across Tasks 9, 13, 14, 19. `SseEvent::with_event` / `with_id` consistent across Tasks 17, 18, 19. `HttpResponse::stream_bytes` / `sse` / `into_hyper_stream` consistent across Tasks 16, 17, 18, 19. `RequestId` / `current_request_id` consistent across Tasks 3, 4, 7, 8, 14. `Context` / `ContextStore` / `CONTEXT` consistent across Task 21 (with the middleware extension in Task 21 referencing the Task 4 `RequestIdMiddleware`).
