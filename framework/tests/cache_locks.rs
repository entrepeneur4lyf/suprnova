...
async fn lock_acquire_succeeds_for_first_caller_only() {
    let s = fresh().await;
    let alice = s.acquire_lock("printer", Duration::from_secs(5)).await.unwrap();
    assert!(alice.is_some(), "first caller acquires lock");

    let bob = s.acquire_lock("printer", Duration::from_secs(5)).await.unwrap();
    assert!(bob.is_none(), "second caller while alice holds the lock");

    s.release_lock("printer", alice.as_ref().unwrap()).await.unwrap();

    let carol = s.acquire_lock("printer", Duration::from_secs(5)).await.unwrap();
...
async fn release_with_wrong_token_does_nothing() {
    let s = fresh().await;
    let alice = s.acquire_lock("printer", Duration::from_secs(5)).await.unwrap().unwrap();
...
async fn lock_expires_after_ttl_and_can_be_reacquired() {
    let s = fresh().await;
    let alice = s.acquire_lock("printer", Duration::from_millis(50)).await.unwrap();
    assert!(alice.is_some());

    tokio::time::sleep(Duration::from_millis(100)).await;

    let bob = s.acquire_lock("printer", Duration::from_secs(5)).await.unwrap();
    assert!(bob.is_some(), "ttl expiry releases the lock for the next caller");
}
...
async fn refresh_extends_ttl_only_for_the_owner() {
    ...
    let refreshed = s.refresh_lock("printer", &alice, Duration::from_millis(200)).await.unwrap();
    assert!(refreshed);

    tokio::time::sleep(Duration::from_millis(100)).await;

    // At 200ms total, alice's original TTL would have expired but refresh kept it alive
    let bob = s.acquire_lock("printer", Duration::from_millis(50)).await.unwrap();
