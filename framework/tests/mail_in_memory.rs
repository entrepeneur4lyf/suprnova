...
impl Mailable for Hello {
    fn mailable_name() -> &'static str { "Hello" }
    fn subject(&self) -> String { format!("Hi {}", self.name) }
    fn text_template_source(&self) -> Option<String> { Some(format!("Hello {}!", self.name)) }
...
async fn in_memory_transport_captures_outgoing_messages() {
    ...
    Mail::to("alice@example.org").send(Hello { name: "Alice".into() }).await.unwrap();
...
async fn mail_send_errors_when_no_transport_bound() {
    ...
    let err = Mail::to("alice@example.org").send(Hello { name: "Alice".into() }).await.unwrap_err();
    let s = format!("{err}");
