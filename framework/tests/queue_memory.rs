...
use suprnova::queue::driver::{Reservation, QueueDriver};
...
async fn pushed_jobs_pop_in_fifo_order_and_ack_removes_them() {
    ...
    d.push(env("J", serde_json::json!({ "i": 1 }))).await.unwrap();
    d.push(env("J", serde_json::json!({ "i": 2 }))).await.unwrap();
    ...
    assert!(r3.is_none(), "queue should be drained after acking everything");
}

#[tokio::test(start_paused = true)]
async fn unacked_messages_reappear_after_visibility_timeout() {
    let d = MemoryQueueDriver::new();
    d.push(env("J", serde_json::json!({ "x": 1 }))).await.unwrap();
    ...
    assert!(reclaimed.is_some(), "message must reappear after visibility timeout");
}
...
async fn nack_returns_message_to_the_head_of_the_queue() {
    let d = MemoryQueueDriver::new();
    d.push(env("J", serde_json::json!({ "x": 1 }))).await.unwrap();
    d.push(env("J", serde_json::json!({ "x": 2 }))).await.unwrap();

    let r1 = d.pop(Duration::from_secs(60)).await.unwrap().unwrap();
    d.nack(&r1.token, Duration::from_millis(0)).await.unwrap();

    let r1_again = d.pop(Duration::from_secs(60)).await.unwrap().unwrap();
    assert_eq!(r1_again.envelope.payload["x"], 1, "nack with zero delay returns the message immediately");
